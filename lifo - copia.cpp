...
int eliminar(){
	...
	//Regresandolos
	for(aux2=cab2; aux2!=NULL;aux2=aux2->sig2)
	{
		aux=(struct nodo*)malloc(sizeof(struct nodo));
		strcpy(aux->placa,aux2->placa2);	
		if (cab==NULL)
		{
	        cab=aux;
	        cab->sig=NULL;
...
int consultar(){
	...
	for(aux=cab; aux!=NULL; aux = aux->sig){
		cont++;
	 cout<<"La placa es "<<cont<<" :"<<aux->placa<<endl;
    }
    return 0;
}
